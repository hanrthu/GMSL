...
from utils import class_num_dict
...
class BaseModel(nn.Module):
    def __init__(
        ...
        if len(class_nums) > 0:
            ...
            print("Warning, you are creating a task with no property predictions!")
    ...
    def forward(self, data: Batch, subset_idx: Optional[Tensor] = None) -> Tensor:
        ...
        if (data.pos > 1e4).any():
            print("Huge positions", data.complex_id)
        ...
        #     if self.enhanced:
        #         s, v, e = self.gnn(s, v, e, edge_index=edge_index, edge_attr=edge_attr, batch=batch)
        #     else:
        #         e_pos = data.e_pos
        ...
        if self.graph_level:
            if self.readout == 'vallina':
                ...
                # 将链从1编号改为从0编号
                chain_pred = scatter(s[(lig_flag != 0).squeeze(), :], index=(chains - 1).squeeze(), dim=0,
                ...
                chain_pred = scatter(s[(lig_flag != 0).squeeze(), :] * self.property_weights,
                                     index=(chains - 1).squeeze(), dim=1, reduce=self.graph_pooling)
                # print("After", y_pred.shape, chain_pred.shape)
                if self.task == 'multi':
                    affinity_pred = [affinity_head(y_pred[i].squeeze()) for i, affinity_head in
                                     enumerate(self.affinity_heads)]
                    property_pred = [property_head(chain_pred[i].squeeze()) for i, property_head in
                                     enumerate(self.property_heads)]
                    for y_property_pred in property_pred:
