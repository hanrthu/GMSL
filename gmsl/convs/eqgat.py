...
from gmsl.modules import (BesselExpansion, ChebyshevExpansion, GaussianExpansion,\
...
class EQGATConv(MessagePassing):
    ...
    def aggregate(
        ...
        # print("Aggregation:", inputs[0].shape, inputs[1].shape, index.shape)
    ...
    def message(
        ...
        d, r = edge_attr
        # print("D:",d.shape, 'R:', r.shape)
        # print("Index:", index)
        de = self.distance_expansion(d)
        dc = self.cutoff_fnc(d)
        de = dc.view(-1, 1) * de
        # print("Message:", sa_i.shape,sa_j.shape,de.shape)
...
class EQGATNoFeatAttnConv(MessagePassing):
    ...
    def forward(
        ...
        s, v = x
        # edge_weights, relevent positions
        d, r = edge_attr
