...
class MultiTaskModel(pl.LightningModule):
    ...
    def forward(self, data: Batch) -> Tuple[Tensor, Tensor]:
        ...
        y_affinity_pred, y_property_pred, y_reaction_pred = self.model(data=data)[0], self.model(data=data)[1], self.model(data=data)[2]
        ...
        y_reaction_true = data.reaction_functions
        y_reaction_mask = data.reaction_masks
        y_reaction_pred = y_reaction_pred[0]
        print("reaction_pred shape", len(y_reaction_pred))
        y_reaction_true = y_reaction_true[(y_reaction_mask == 1).sum(dim=1) > 0, :]
        
        y_reaction_pred = y_reaction_pred[(y_reaction_mask == 1).sum(dim=1) > 0, :]
        ...
        for j, (affinity_name, affinity_num) in enumerate(self.affinity_info.items()):
            ...
            # print("curr true:",curr_true)
            # print("curr mask:",curr_mask)
        ...
        return y_affinity_preds, y_property_preds, y_affinity_trues, y_property_trues, y_reaction_pred, y_reaction_true
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_affinity_preds, y_property_preds, y_affinity_trues, y_property_trues, y_reaction_pred, y_reaction_true = self(batch)
        bce_losses = 0
        l2_losses = 0
        ...
        mce_loss = self.mce(y_reaction_pred, y_reaction_true.view(-1,)) * int(len(y_property_pred)!=0)
        print("train mce loss", mce_loss)
        self.log("train_mce_reaction", mce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        if not bce_losses == 0 and not l2_losses == 0 and not mce_loss == 0:
            # print("BCE Loss:", bce_loss, "L2 Loss:", l2_loss)
            loss = 10 * bce_losses + l2_losses + 10 * mce_loss
        elif not bce_losses == 0 and not l2_losses == 0:
            # print("BCE Loss:", bce_loss)
            loss = 10 * bce_losses + l2_losses
        elif not l2_losses == 0 and not mce_loss == 0:
            # print("L2 Loss:", l2_loss)
            loss = l2_losses + 10 * mce_loss
        elif not bce_losses == 0 and not mce_loss == 0:
            loss =  10 * bce_losses + 10 * mce_loss
        ...
        training_step_output['mce_total'] = mce_loss if mce_loss != 0 else -1
        if torch.isnan(loss):
        ...
        self.log("train_mce", mce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append({"affinity_true": y_affinity_trues, 'affinity_pred': y_affinity_preds, 'property_true': y_property_trues, "property_pred": y_property_preds, "reaction_pred": y_reaction_pred, "reaction_true": y_reaction_true})
    ...
    def test_step(self, batch, batch_idx):
        ...
        #     y_pred, y_true, _, _ = self(batch)
        y_affinity_preds, y_property_preds, y_affinity_trues, y_property_trues, y_reaction_pred, y_reaction_true = self(batch)
        self.test_step_outputs.append({"affinity_true": y_affinity_trues,"affinity_pred": y_affinity_preds,"property_true": y_property_trues,"property_pred": y_property_preds, "reaction_pred": y_reaction_pred, "reaction_true": y_reaction_true})
    ...
    def on_train_epoch_end(self):
        ...
        mce_loss = torch.stack([x["mce_total"] for x in self.training_step_outputs if x['mce_total'] != -1]).mean().item()
        to_print = (
        ...
        self.log("train_mce", mce_loss, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
    ...
    def on_validation_epoch_end(self):
        ...
        reaction_pred = torch.concat([x["reaction_pred"] for x in self.validation_step_outputs])
        reaction_true = torch.concat([x["reaction_true"] for x in self.validation_step_outputs])
        mce_loss = self.mce(reaction_pred, reaction_true.view(-1,)).item()
        print("mce_loss type",mce_loss)
        acc = self.cal_acc(reaction_pred, reaction_true)
        # val_loss = 10 * mce_loss
        ...
        val_loss = l2_losses + 10 * bce_losses + 10 * mce_loss
        print("val_loss type",type(val_loss))
    ...
    def on_test_epoch_end(self):
        ...
        reaction_pred = torch.concat([x["reaction_pred"] for x in self.test_step_outputs])
        reaction_true = torch.concat([x["reaction_true"] for x in self.test_step_outputs])
        mce_loss = self.mce(reaction_pred, reaction_true.view(-1,))
        acc = self.cal_acc(reaction_pred, reaction_true)
        self.log("val_acc_reaction", round(acc,4),on_step=False,on_epoch=True,prog_bar=False, sync_dist=True)
...
class PropertyModel(pl.LightningModule):
    ...
    def forward(self, data: Batch) -> Tuple[Tensor, Tensor]:
        ...
        print("y_property_pred:", y_property_pred.shape)
        y_property_true = data.functions
        y_property_mask = data.valid_masks
        # print("in forward:")
        print("y_property_true:", y_property_true.shape)
        ...
        print("y_property_true after process:", y_property_true.shape)
        print("y_property_pred after process:", y_property_pred.shape)
...
class ReactionModel(pl.LightningModule):
    def __init__(
        ...
        print("Initializing Reaction Model...")
        self.save_hyperparameters(args)
    ...
    def forward(self, data: Batch) -> Tuple[Tensor, Tensor]:
        y_reaction_pred = self.model(data)[0]
        y_reaction_true = data.reaction_functions
        y_reaction_mask = data.reaction_masks
        print("y_reaction_pred shape ",y_reaction_pred.shape)
        print("y_reaction_true shape ",y_reaction_true.shape)
        print("y_reaction_mask shape ",y_reaction_mask.shape)
        print("y_reaction_true ",y_reaction_true)
        y_reaction_true = y_reaction_true[(y_reaction_mask == 1).sum(dim=1) > 0, :]
        
        y_reaction_pred = y_reaction_pred[(y_reaction_mask == 1).sum(dim=1) > 0, :]
        # print("y_property_pred after process:", y_property_pred)
        return y_reaction_pred, y_reaction_true
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print("y_property_pred:",y_property_pred)
        # print("y_property_true",y_property_true)
        mce_loss = self.mce(y_property_pred, y_property_true.view(-1,)) * int(len(y_property_pred)!=0)
        loss = 10 * mce_loss
        self.log("train_loss", loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.log("train_mce", mce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.training_step_outputs.append({"loss": loss, "mce": mce_loss})
        return {"loss": loss, "mce": mce_loss}

    def validation_step(self, batch, batch_idx):
        y_reaction_pred, y_reaction_true = self(batch)
        # print("in validation_step:")
        # print("y_property_pred:",y_property_pred)
        # print("y_property_true",y_property_true)
        bce_loss = self.mce(y_reaction_pred, y_reaction_true.view(-1,)) * int(len(y_reaction_pred)!=0)
        self.log("val_bce", bce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.log("val_loss", 10 * bce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.validation_step_outputs.append({"reaction_true": y_reaction_true, "reaction_pred": y_reaction_pred})
        return {
                "reaction_true": y_reaction_true,
                "reaction_pred": y_reaction_pred
                }

    def test_step(self, batch, batch_idx):
        y_reaction_pred, y_reaction_true = self(batch)
        self.test_step_outputs.append({"reaction_true": y_reaction_true,"reaction_pred": y_reaction_pred})
        return {
            "reaction_true": y_reaction_true,
            "reaction_pred": y_reaction_pred
            }
    ...
    def on_validation_epoch_end(self):
        reaction_pred = torch.concat([x["reaction_pred"] for x in self.validation_step_outputs])
        reaction_true = torch.concat([x["reaction_true"] for x in self.validation_step_outputs])
        bce_loss = self.mce(reaction_pred, reaction_true.view(-1,))
        if self.task in ['reaction']:
            acc = self.cal_acc(reaction_pred, reaction_true)
            val_loss = 10 * bce_loss
            to_print = (
            f"{self.current_epoch:<10}: "
            f"ACC_{self.task}: {round(acc, 4)}"
            )
            self.res = {'acc'.format(self.task): acc}
            # print("Self.Res:", self.res)
            print("VALID:", to_print)
            self.log("val_loss", val_loss, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
            self.log("val_acc_all", acc, on_step=False, on_epoch=True, prog_bar=False,sync_dist=True)
        self.validation_step_outputs.clear()
    ...
    def on_test_epoch_end(self):
        ...
        if self.task in ['reaction']:
            ...
            self.res = {'acc_all'.format(self.task): acc}
            # print("Self.Res:", self.res)
            print("TEST:", to_print)
            self.log("test_acc_all", acc, on_step=False, on_epoch=True, prog_bar=False,sync_dist=True)
        self.test_step_outputs.clear()
        return acc
    
class FoldModel(pl.LightningModule):
    def __init__(
        self,
        args,
        sdim: int = 128,
    ...
    def forward(self, data: Batch) -> Tuple[Tensor, Tensor]:
        print(data.__len__())
        y_fold_pred = self.model(data)[0]
        y_fold_true = data.fold_functions
        y_fold_mask = data.fold_masks
        # print("y_fold_pred shape ",y_fold_pred.shape)
        # print("y_fold_true shape ",y_fold_true.shape)
        # print("y_fold_mask shape ",y_fold_mask.shape)
        # print("y_fold_true ",y_fold_true)
        y_fold_true = y_fold_true[(y_fold_mask == 1).sum(dim=1) > 0, :]
        
        y_fold_pred = y_fold_pred[(y_fold_mask == 1).sum(dim=1) > 0, :]
        # print("y_property_pred after process:", y_property_pred)
        return y_fold_pred, y_fold_true


    def training_step(self, batch, batch_idx):
        y_fold_pred, y_fold_true = self(batch)
        # print("in training_step:")
        # print("y_property_pred:",y_property_pred)
        # print("y_property_true",y_property_true)
        mce_loss = self.mce(y_fold_pred, y_fold_true.view(-1,)) * int(len(y_fold_pred)!=0)
        loss = 10 * mce_loss
        self.log("train_loss", loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.log("train_mce", mce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.training_step_outputs.append({"loss": loss, "mce": mce_loss})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # print("y_property_pred:",y_property_pred)
        # print("y_property_true",y_property_true)
        bce_loss = self.mce(y_fold_pred, y_fold_true.view(-1,)) * int(len(y_fold_pred)!=0)
        self.log("val_bce", bce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.log("val_loss", 10 * bce_loss, on_step=True, on_epoch=False, prog_bar=True, sync_dist=True)
        self.validation_step_outputs.append({"fold_true": y_fold_true, "fold_pred": y_fold_pred})
        return {
                "fold_true": y_fold_true,
                "fold_pred": y_fold_pred
                }

    def test_step(self, batch, batch_idx):
        y_fold_pred, y_fold_true = self(batch)
        self.test_step_outputs.append({"fold_true": y_fold_true,"fold_pred": y_fold_pred})
        return {
            "fold_true": y_fold_true,
            "fold_pred": y_fold_pred
            }
    ...
    def on_validation_epoch_end(self):
        fold_pred = torch.concat([x["fold_pred"] for x in self.validation_step_outputs])
        fold_true = torch.concat([x["fold_true"] for x in self.validation_step_outputs])
        bce_loss = self.mce(fold_pred, fold_true.view(-1,))
        if self.task in ['fold']:
            acc = self.cal_acc(fold_pred, fold_true)
            val_loss = 10 * bce_loss
            to_print = (
            f"{self.current_epoch:<10}: "
            f"ACC_{self.task}: {round(acc, 4)}"
    ...
    def on_test_epoch_end(self):
        fold_pred = torch.concat([x["fold_pred"] for x in self.test_step_outputs])
        fold_true = torch.concat([x["fold_true"] for x in self.test_step_outputs])
        if self.task in ['fold']:
            acc = self.cal_acc(fold_pred, fold_true)
            to_print = (
            f"{self.current_epoch:<10}: "
